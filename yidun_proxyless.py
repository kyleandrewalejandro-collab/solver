...
def initialize_global_model():
    ...
    with _model_lock:
        ...
        if not os.path.exists(model_path):
            logger.warning("Model file net.pkl not found - running in detection-fallback mode")
            # Create stub state that passes validation
            _model_state = {
                'net': None,
                'anchors': emergency_fallback(),
                'fallback_mode': True
            }
            return _model_state

        try:
            # net.pkl was pickled with Mini in __main__
            # use custom unpickler to remap __main__.Mini -> this module's Mini
            import pickle
            import io
            ...
            class _TorchRemapLoad:
                """Loads a torch checkpoint using _Remapper for pickle resolution."""
                pass

            with open(model_path, 'rb') as f:
                raw = f.read()

            # torch saves with zipfile for newer checkpoints; fall back to legacy pickle
            try:
                import zipfile
                if zipfile.is_zipfile(io.BytesIO(raw)):
                    state = torch.load(
                        ...
                        pickle_module=type('_pm', (), {
                            'load': lambda f, **kw: _Remapper(f).load(),
                            'Unpickler': _Remapper,
                            'dump': pickle.dump,
                        })()
                    )
                else:
                    state = _Remapper(io.BytesIO(raw)).load()
            except Exception:
                state = _Remapper(io.BytesIO(raw)).load()

            # FORGIVING: If state is corrupted, fall back to emergency
            if not isinstance(state, dict):
                logger.warning(f"Checkpoint corrupted (got {type(state).__name__}), using fallback detection")
                _model_state = {
                    'net': None,
                    'anchors': emergency_fallback(),
                    'fallback_mode': True
            ...
            if 'net' in state:
                ...
                # Forgiving net validation
                if not isinstance(net_obj, nn.Module):
                    logger.warning(f"Model invalid (got {type(net_obj).__name__}), using fallback")
                    _model_state = {
                        'net': None,
                        'anchors': emergency_fallback(),
                        'fallback_mode': True
            ...
            logger.error(f"Model load failed: {e}, falling back to detection mode")
            _model_state = {
                'net': None,
                'anchors': emergency_fallback(),
                'fallback_mode': True
...
def main():
    ...
    if not model_state:
        logger.error("Critical: Model state initialization returned None")
        return

    # Model is optional in railway deployment
    fallback_mode = model_state.get('fallback_mode', False)
    if fallback_mode:
        logger.warning("Running in fallback detection mode (no neural net)")
    else:
        logger.success("Neural model loaded")
...
def initialize_global_model():
    ...
    with _model_lock:
        ...
        try:
            ...
            # CRITICAL: Validate state is dict, not primitive
            if not isinstance(state, dict):
                logger.error(f"Checkpoint load failed: expected dict, got {type(state).__name__}. File may be corrupted or checkpoint format mismatch.")
                return None

            if 'net' in state:
                net_obj = state['net']
                
                # Validate net is model object, not ID or primitive
                if not isinstance(net_obj, nn.Module):
                    logger.error(f"Model validation failed: net is {type(net_obj).__name__}, expected nn.Module")
                    return None
