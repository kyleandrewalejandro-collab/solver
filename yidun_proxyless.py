...
def initialize_global_model():
    ...
    with _model_lock:
        ...
        try:
            ...
            # Validate state is dict before accessing keys
            if not isinstance(state, dict):
                logger.error(f"State load error: expected dict, got {type(state).__name__}")
                return None

            if 'net' in state:
                net_obj = state['net']
                
                # Validate net is actual model, not primitive
                if not isinstance(net_obj, nn.Module):
                    logger.error(f"Invalid net type: {type(net_obj).__name__}, expected nn.Module")
                    return None
